 ...
 * Return: bytes read
 */
ssize_t ibuf(info_t *info, char **buf, size_t *len)
{
	ssize_t r = 0;
	size_t len_p = 0;
	...
	if (!*len) /* if nothing left in the buffer, fill it */
	{
		...
		*buf = NULL;
		...
		if (r > 0)
		{
			...
			record_history_list(info, *buf, info->histcount++);
...
ssize_t g_input(info_t *info)
{
	...
	if (len) /* we have commands left in the chain buffer */
	{
		...
		{
			if (is_chain_internal(info, buf, &j))
				break;
			j++;
		}
 ...
 * Return: s
 */
int gl(info_t *info, char **ptr, size_t *length)
{
