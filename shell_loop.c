...
int hsh_loop(info_t *info, char **av)
{
	...
	while (r != -1 && builtin_ret != -2)
	{
		...
		if (r != -1)
		{
			initialize_information(info, av);
			...
			_putchar('\n');
		release_information(info, 0);
	}
	load_history(info);
	release_information(info, 1);
...
void find_cmd(info_t *info)
{
	...
	else
	{
		...
		{
			info->status = 127;
			_print_error_message(info, "not found\n");
		}
	}
}
...
void fork_cmd(info_t *info)
{
	...
	else
	{
		...
		if (WIFEXITED(info->status))
		{
			...
			if (info->status == 126)
				_print_error_message(info, "Permission denied\n");
		}
	}
}
